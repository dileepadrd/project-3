...
contract SupplyChain {
    ...
    function confirmTransaction(bytes32 _id) public {
        ...
        // Verify that the sender is the buyer
        require(
            msg.sender == contractDetails.buyer,
            "You are not authorized to confirm the transaction"
        );

        // Verify that the delivery is done and the payment is done
        require(
            contractDetails.deliveryDone == true &&
                contractDetails.paymentDone == true,
            "Delivery and payment are not complete"
        );
